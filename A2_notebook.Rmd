...
graphsMetrics <- function(titles)
{
  ...
  degrees <- graphsDegCentralities(titles)
  precisions <- graphsPrecisions(titles)
  recalls <- graphsRecalls(titles)
  f1s <- 2 * precisions * recalls / (precisions + recalls)
  
  return(rbind(metrics[1:5,], MECs, degrees, precisions, recalls, F1=f1s))
}
...
graphsDegCentralities <- function(titles)
{
  ntitles <- length(titles)
  metrics <- data.frame(matrix(
    rep(0, len=ntitles),
    nrow=1,
    ncol=ntitles,
    dimnames=list("Degree Centrality", titles)
  ))
...
graphsPrecisions <- function(titles)
{
  ntitles <- length(titles)
  metrics <- data.frame(matrix(
    rep(0, len=ntitles),
    nrow=1,
    ncol=ntitles,
    dimnames=list("Precision", titles)
  ))
  ...
  for (i in 1:ntitles)
  {
    for (j in 1:length(graphs[[i]]))
    {
      ...
      for (k in 1:K)
      {
        from <- learned.arcs[k,"from"]
        tos <- true.arcs[true.arcs[,"from"] == from,"to"]
        metrics[1,i] <- metrics[1,i] + (learned.arcs[k,"to"] %in% tos) / K
      }
    }
    metrics[1,i] <- metrics[1,i] / length(graphs[[i]])
  }
  ...
  for (i in 1:ntitles)
  {
    for (j in 1:length(graphs[[i]]))
    {
      ...
      for (k in 1:K)
      {
        ...
        metrics[1,i] <- metrics[1,i] + (true.arcs[k,"to"] %in% tos) / K
      }
    }
    metrics[1,i] <- metrics[1,i] / length(graphs[[i]])
  }
...
graphs <- graphs[1]
years.list <- years.list[1]
alphas = c(0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.5)
# scaled.data$weathersit <- scale(as.numeric(data$weathersit))[,1]
...
run.networks(n.iters, FALSE, pc.stable, factor.data, alpha=alphas[3])
run.networks(n.iters, FALSE, pc.stable, factor.data, alpha=alphas[4])
run.networks(n.iters, FALSE, pc.stable, factor.data, alpha=alphas[5])
run.networks(n.iters, FALSE, pc.stable, factor.data, alpha=alphas[6])
run.networks(n.iters, FALSE, pc.stable, factor.data, alpha=alphas[7])
