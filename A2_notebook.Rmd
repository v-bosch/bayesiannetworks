...
mse <- function(g)
{
  ...
  MSE <- mean((data$casual + data$registered - years$casual - years$registered)^2)
  return(MSE)
}
...
write("MSE on the total number of users:", stdout())
write(mse(g), stdout())
...
write("\nArcs in the DAG:", stdout())
write(length(arcs(manual.g)) / 2, stdout())

write("MSE on the total number of users:", stdout())
write(mse(manual.g), stdout())
...
```
```{r}
dists <- graphsDists(c("aic-cg","bic-cg","loglik-cg","pred-loglik-cg","manual"))
