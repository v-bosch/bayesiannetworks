...
graphsMetrics <- function(titles) {
  ...
  return(rbind(
    arcs.dists, MECs, degrees, confMatrices, Precision=micro.precisions,
...
graphsConfMatrices <- function(titles) {
  ...
  for (i in 1:ntitles) {
    for (j in 1:length(graphs[[i]])) {
      ...
      metrics[3,i] <- metrics[3,i] + narcs(graphs[[1]][[1]]) - correct.arcs
      metrics[4,i] <- metrics[4,i] + 91 + correct.arcs - narcs(graphs[[i]][[j]]) - narcs(graphs[[1]][[1]])
    }
  }
...
run.networks(n.iters, 2 - random, hc, factor.data, score=scores[2])
run.networks(n.iters, 2 - random, hc, factor.data, score=scores[3])
run.networks(n.iters, 2 - random, hc, factor.data80, score=scores[4], newdata=factor.data20)
...
n.iters = 1
for (alpha in alphas) {
  run.networks(n.iters, FALSE, pc.stable, factor.data, alpha=alpha)
}
