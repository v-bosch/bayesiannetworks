...
graphsMetrics <- function(titles)
{
  ...
  return(rbind(
    metrics[1:5,], MECs, degrees, confMatrices, Precision=micro.precisions,
  ...
  for (i in 1:ntitles)
  {
    for (j in 1:length(graphs[[1]]))
    {
      ...
      for (k in 1:K)
      {
        ...
        metrics[1,i] <- metrics[1,i] + true.arcs[k,"to"] %in% froms
      }
    }
    metrics[1,i] <- metrics[1,i] / length(graphs[[i]])
  }
...
graphsConfMatrices <- function(titles)
{
  ...
  for (i in 1:ntitles)
  {
    for (j in 1:length(graphs[[i]]))
    {
      ...
      metrics[2,i] <- metrics[2,i] + nrow(learned.arcs) - correct.arcs
      metrics[3,i] <- metrics[3,i] + nrow(true.arcs) - correct.arcs
      metrics[4,i] <- metrics[4,i] + 91 - correct.arcs - nrow(learned.arcs) -  nrow(true.arcs) + 2 * correct.arcs
    }
  }
