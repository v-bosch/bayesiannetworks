...
graphsPageRanks <- function(titles)
{
  ...
  for (i in 1:ntitles)
  {
    ...
    orderPageRanks <- order(match(colnames(data), graphPageRanks$from))
    pageRanks[orderPageRanks,i] <- graphPageRanks$rank
  }
...
```{r}
g <- hc(factor.data, score="aic-cg") #, k=1)
save.network(g)
plot(g)
```

```{r}
g <- hc(factor.data, score="bic-cg")
save.network(g)
plot(g)
```

```{r}
g <- hc(factor.data, score="loglik-cg")
save.network(g)
plot(g)
```
```{r}
...
factor.data.prop <- factor.data100[1:cut,]
factor.data.not.prop <- factor.data100[(cut + 1):nrow(data),]

g <- hc(factor.data.prop, score="pred-loglik-cg", newdata=factor.data.not.prop)
save.network(g)
plot(g)
```
