...
save.network <- function(g, i, pred)
{
  ...
  if (pred)
  {
    ...
    fit <- bn.fit(g, data)  # use non-scaled numeric version of data for prediction
...
graphsMetrics <- function(titles)
{
  ...
  metrics <- data.frame(matrix(
    c(rep(0, len=5*ntitles), rep(Inf, len=ntitles)),
    byrow=TRUE,
    nrow=6,
    ncol=ntitles,
    dimnames=list(c("arcs", "mse.cas", "mse.reg", "mse.tot", "bestMSEidx", "0"), titles)
  ))
  ...
  for (i in 1:ntitles)
  {
    for (j in 1:length(years.list[[i]]))
    {
      ...
      metrics[1:4,i] <- metrics[1:4,i] + (arcs.mses - metrics[1:4,i]) / j
      
      if (arcs.mses[4] < metrics[6,i])
      {
        metrics[5,i] <- j
        metrics[6,i] <- arcs.mses[4]
      }
    }
  }
...
{
  ntitles <- length(titles)
  dists <- data.frame(matrix(
    c(rep(0, len=4*ntitles), rep(Inf, len=2*ntitles)),
    byrow=TRUE,
    nrow=6,
    ncol=ntitles,
    dimnames=list(c("SID", "SHD", "bestSIDidx", "bestSHDidx", "0", "1"), titles)
  ))
  
  true.graph <- graphs[[1]][[1]]
  for (i in 1:ntitles)
  {
    for (j in 1:length(graphs[[i+1]]))
    {
      learned.graph <- graphs[[i+1]][[j]]
      sid.shd <- c(
        structIntervDist(amat(true.graph), amat(learned.graph))$sid,
        hammingDist(amat(true.graph), amat(learned.graph))
      )
      dists[1:2,i] <- dists[1:2,i] + (sid.shd - dists[1:2,i]) / j
      
      if (sid.shd[1] < dists[5,i])
      {
        dists[3,i] <- j
        dists[5,i] <- sid.shd[1]
      }
      if (sid.shd[2] < dists[6,i])
      {
        dists[4,i] <- j
        dists[6,i] <- sid.shd[2]
      }
...
```{r}
# set scores that will be used and initialize list that will hold predictions
...
run.networks(n.iters, TRUE, hc, factor.data80, score=scores[4], newdata=factor.data20)

# compute and show the metrics
metrics <- graphsMetrics(c("manual", scores)); print(metrics)
dists <- graphsDists(scores); print(dists)
pageRanks <- graphsPageRanks(c("manual", scores)); print(pageRanks)
```

```{r}
for (i in 1:length(scores))
{
  bestIdx <- dists["bestSIDidx",i]
  plot(graphs[[i]][[bestIdx]], main=sprintf("Best %s network", scores[i]))
}
```
```{r}
for (i in 1:length(c("manual", scores)))
{
  bestIdx <- metrics["bestMSEidx",i]
  title <- sprintf("Best %s predictions MSE=%.0f",
    c("manual", scores)[i],
    metrics["mse.tot", i]
  )
  plot(plot.preds(i, bestIdx, title))
}
```
...
# compute and show the metrics
alpha.titles = paste0("alpha=", alphas)
dists <- graphsDists(alpha.titles); print(dists)
pageRanks <- graphsPageRanks(c("manual", alpha.titles)); print(pageRanks)
```
...
{
  bestIdx <- dists["bestSIDidx",i]
  plot(graphs[[i]][[bestIdx]], main=sprintf("Best alpha=%s network", alphas[i]))
}
```
