...
MSEs <- graphsMSEs(c("manual", scores)); print(MSEs)
arcs.dists <- graphsArcsDists(scores); print(arcs.dists)
...
{
  bestIdx <- if (i == 1) 1 else arcs.dists["bestSIDidx",i-1]
  plot(graphs[[i]][[bestIdx]], main=sprintf("Best %s network", c("Manual", scores)[i]))
}
```
```{r}
for (i in 1:length(c("manual", scores)))
...
arcs.dists <- graphsArcsDists(alpha.titles); print(arcs.dists)
pageRanks <- graphsPageRanks(c("manual", alpha.titles)); print(pageRanks)
```

```{r}
for (i in 1:length(c("manual", alphas)))
{
  bestIdx <- if (i == 1) 1 else arcs.dists["bestSIDidx",i-1]
  plot(graphs[[i]][[bestIdx]], main=sprintf("Best %s network", c("Manual", alpha.titles)[i]))
}
```
